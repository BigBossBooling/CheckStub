...
def generate_check_stub_route():
    if request.method == 'POST':
        try:
            ...
            # --- Data Transformation for Earnings & Deductions ---
            # The generate_check_stub function expects 'earnings' and 'deductions' as lists of dicts.
            # We need to parse them from the fixed form fields.
            parsed_data = form_data.copy() # Start with all flat fields

            parsed_data['earnings'] = []
            if form_data.get('earning1_desc') and form_data.get('earning1_current'): # Basic check for presence
                parsed_data['earnings'].append({
                    'description': form_data.get('earning1_desc'),
                    'hours': form_data.get('earning1_hours', ''),
                    'rate': form_data.get('earning1_rate', ''),
                    'current': form_data.get('earning1_current'),
                    'ytd': form_data.get('earning1_ytd', '')
                })
            if form_data.get('earning2_desc') and form_data.get('earning2_current'):
                parsed_data['earnings'].append({
                    'description': form_data.get('earning2_desc'),
                    'hours': form_data.get('earning2_hours', ''),
                    'rate': form_data.get('earning2_rate', ''),
                    'current': form_data.get('earning2_current'),
                    'ytd': form_data.get('earning2_ytd', '')
                })
            # Remove flat earning fields after parsing if they conflict with list name (not strictly necessary here)

            parsed_data['deductions'] = []
            if form_data.get('deduction1_desc') and form_data.get('deduction1_current'):
                 parsed_data['deductions'].append({
                    'description': form_data.get('deduction1_desc'),
                    'current': form_data.get('deduction1_current'),
                    'ytd': form_data.get('deduction1_ytd', '')
                })
            if form_data.get('deduction2_desc') and form_data.get('deduction2_current'):
                 parsed_data['deductions'].append({
                    'description': form_data.get('deduction2_desc'),
                    'current': form_data.get('deduction2_current'),
                    'ytd': form_data.get('deduction2_ytd', '')
                })
